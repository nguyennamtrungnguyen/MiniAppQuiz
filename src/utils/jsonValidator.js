...
export function validateQuizJSON(data) {
  ...
  if (!Array.isArray(data)) {
    return { valid: false, errors: ['Dữ liệu phải là một mảng (Array)'], data: [] };
  }

  if (data.length === 0) {
    return { valid: false, errors: ['Mảng dữ liệu rỗng, không có câu hỏi nào'], data: [] };
  }
  ...
  data.forEach((item, index) => {
    ...
    if (!item.options || typeof item.options !== 'object') {
      ...
      validAnswers.forEach(key => {
        if (!item.options[key] || typeof item.options[key] !== 'string' || item.options[key].trim() === '') {
    ...
    if (!item.answer) {
      ...
      errors.push(`Câu hỏi số ${pos}: 'answer' phải là A, B, C hoặc D (hiện tại: '${item.answer}')`);
    }
  });
